...
#if defined(NDEBUG)
#error "bitcoinpay cannot be compiled without assertions."
#endif
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	if (nHeight == 0) {
		  ...
		  ret = blockValue  / 100 * 75;// 75% for masternode
...
bool IsInitialBlockDownload()
{
    ...
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 ||
                  pindexBestHeader->GetBlockTime() < GetTime() - 6 * 60 * 60) && chainActive.Height() == 350; // ~144 blocks behind -> 2 x fork detection ; // ~144 blocks behind -> 2 x fork detection time
    if (!state)
        lockIBDState = true;
    return state;
}
...
{
    RenameThread("bitcoinpay-scriptch");
