...
#else
    const char* pszModule = "bitcoinpay";
#endif
    if (pex)
        return strprintf(
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\bitcoinpay
// Windows >= Vista: C:\Users\Username\AppData\Roaming\bitcoinpay
// Mac: ~/Library/Application Support/bitcoinpay
// Unix: ~/.bitcoinpay
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "bitcoinpay";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "bitcoinpay.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "bitcoinpayd.pid"));
