...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::BPAY)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("BPAY")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    if (nMaxToAnonymize > nAnonymizeBitcoinPayAmount * COIN) nMaxToAnonymize = nAnonymizeBitcoinPayAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeBitcoinPayAmount * COIN) {
