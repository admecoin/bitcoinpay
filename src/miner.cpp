...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("bitcoinpayMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("bitcoinpayMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("bitcoinpayMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcoinpay-miner");
...
///////|| !masternodeSync.IsSynced()
        ...
        LogPrintf("Running bitcoinpayMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
