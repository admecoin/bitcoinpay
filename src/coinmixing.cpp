...
// The main object for accessing CoinMixing
CCoinMixingPool obfuScationPool;
// A helper object for signing messages from Masternodes
CObfuScationSigner obfuScationSigner;
// The current CoinMixings in progress on the network
std::vector<CCoinMixingQueue> vecCoinMixingQueue;
// Keep track of the used Masternodes
std::vector<CTxIn> vecMasternodesUsed;
// Keep track of the scanning errors I've seen
map<uint256, CCoinMixingBroadcastTx> mapCoinMixingBroadcastTxes;
// Keep track of the active Masternode
CActiveMasternode activeMasternode;

/* *** BEGIN COINMIXING MAGIC - BPAY **********
    Copyright (c) 2014-2015, Dash Developers
        eduffield - evan@dashpay.io
        udjinm6   - udjinm6@dashpay.io
*/

void CCoinMixingPool::ProcessMessageCoinMixing(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    if (fLiteMode) return; //disable all CoinMixing/Masternode related functionality
    if (!masternodeSync.IsBlockchainSynced()) return;

    if (strCommand == "dsa") { //CoinMixing Accept Into Pool
    ...
    } else if (strCommand == "dsq") { //CoinMixing Queue
        TRY_LOCK(cs_coinmixing, lockRecv);
        if (!lockRecv) return;
        ...
        if (dsq.ready) {
            ...
            if (state == POOL_STATUS_QUEUE) {
                LogPrint("coinmixing", "CoinMixing queue is ready - %s\n", addr.ToString());
                PrepareCoinMixingDenominate();
            }
        } else {
            BOOST_FOREACH (CCoinMixingQueue q, vecCoinMixingQueue) {
                if (q.vin == dsq.vin) return;
            }

            LogPrint("coinmixing", "dsq last %d last2 %d count %d\n", pmn->nLastDsq, pmn->nLastDsq + mnodeman.size() / 5, mnodeman.nDsqCount);
            //don't allow a few nodes to dominate the queuing process
            if (pmn->nLastDsq != 0 &&
                pmn->nLastDsq + mnodeman.CountEnabled(ActiveProtocol()) / 5 > mnodeman.nDsqCount) {
                LogPrint("coinmixing", "dsq -- Masternode sending too many dsq messages. %s \n", pmn->addr.ToString());
                return;
            }
            mnodeman.nDsqCount++;
            ...
            LogPrint("coinmixing", "dsq - new CoinMixing queue object - %s\n", addr.ToString());
            vecCoinMixingQueue.push_back(dsq);
            dsq.Relay();
            dsq.time = GetTime();
        }
        ...
        //check it like a transaction
        {
            ...
            BOOST_FOREACH (const CTxIn i, in) {
                ...
                LogPrint("coinmixing", "dsi -- tx in %s\n", i.ToString());
            ...
            if (nValueIn > COINMIXING_POOL_MAX) {
                LogPrintf("dsi -- more than CoinMixing pool max! %s\n", tx.ToString());
                errorID = ERR_MAXIMUM;
                pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), MASTERNODE_REJECTED, errorID);
                return;
            }
    ...
    } else if (strCommand == "dssu") { //CoinMixing status update
        if (pfrom->nVersion < ActiveProtocol()) {
            return;
        }
        ...
        LogPrint("coinmixing", "dssu - state: %i entriesCount: %i accepted: %i error: %s \n", state, entriesCount, accepted, GetMessageByID(errorID));

        if ((accepted != 1 && accepted != 0) && sessionID != sessionIDMessage) {
            LogPrintf("dssu - message doesn't match current CoinMixing session %d %d\n", sessionID, sessionIDMessage);
            return;
        }

        StatusUpdate(state, entriesCount, accepted, errorID, sessionIDMessage);

    } else if (strCommand == "dss") { //CoinMixing Sign Final Tx
        ...
        BOOST_FOREACH (const CTxIn item, sigs) {
            ...
            LogPrint("coinmixing", " -- sigs count %d %d\n", (int)sigs.size(), count);
            count++;
        }
    ...
    } else if (strCommand == "dsf") { //CoinMixing Final tx
        if (pfrom->nVersion < ActiveProtocol()) {
            return;
        }
        ...
        if (sessionID != sessionIDMessage) {
            LogPrint("coinmixing", "dsf - message doesn't match current CoinMixing session %d %d\n", sessionID, sessionIDMessage);
            return;
        }
    ...
    } else if (strCommand == "dsc") { //CoinMixing Complete
        ...
        if (sessionID != sessionIDMessage) {
            LogPrint("coinmixing", "dsc - message doesn't match current CoinMixing session %d %d\n", obfuScationPool.sessionID, sessionIDMessage);
            return;
        }
...
bool CCoinMixingPool::SetCollateralAddress(std::string strAddress)
{
    CBitcoinAddress address;
    if (!address.SetString(strAddress)) {
        LogPrintf("CCoinMixingPool::SetCollateralAddress - Invalid CoinMixing collateral address\n");
        return false;
    }
...
//
// Unlock coins after CoinMixing fails or succeeds
//
void CCoinMixingPool::UnlockCoins()
{
    while (true) {
...
std::string CCoinMixingPool::GetStatus()
{
    ...
    case POOL_STATUS_ACCEPTING_ENTRIES:
        if (entriesCount == 0) {
            ...
            return _("CoinMixing request complete:") + " " + _("Your transaction was accepted into the pool!");
        } else {
            std::string suffix = "";
    ...
    case POOL_STATUS_ERROR:
        return _("CoinMixing request incomplete:") + " " + lastMessage + " " + _("Will retry...");
    case POOL_STATUS_SUCCESS:
        return _("CoinMixing request complete:") + " " + lastMessage;
...
//
// Check the CoinMixing progress and send client updates if a Masternode
//
void CCoinMixingPool::Check()
{
    if (fMasterNode) LogPrint("coinmixing", "CCoinMixingPool::Check() - entries count %lu\n", entries.size());
    //printf("CCoinMixingPool::Check() %d - %d - %d\n", state, anonTx.CountEntries(), GetTimeMillis()-lastTimeChanged);

    if (fMasterNode) {
        LogPrint("coinmixing", "CCoinMixingPool::Check() - entries count %lu\n", entries.size());

        // If entries is full, then move on to the next phase
        if (state == POOL_STATUS_ACCEPTING_ENTRIES && (int)entries.size() >= GetMaxPoolTransactions()) {
            LogPrint("coinmixing", "CCoinMixingPool::Check() -- TRYING TRANSACTION \n");
    ...
    if (state == POOL_STATUS_FINALIZE_TRANSACTION) {
        LogPrint("coinmixing", "CCoinMixingPool::Check() -- FINALIZE TRANSACTIONS\n");
        ...
        if (fMasterNode) {
            ...
            LogPrint("coinmixing", "Transaction 1: %s\n", txNew.ToString());
    ...
    if (fMasterNode && state == POOL_STATUS_SIGNING && SignaturesComplete()) {
        LogPrint("coinmixing", "CCoinMixingPool::Check() -- SIGNING\n");
    ...
    if ((state == POOL_STATUS_ERROR || state == POOL_STATUS_SUCCESS) && GetTimeMillis() - lastTimeChanged >= 10000) {
        LogPrint("coinmixing", "CCoinMixingPool::Check() -- timeout, RESETTING\n");
        UnlockCoins();
        SetNull();
...
void CCoinMixingPool::CheckFinalTransaction()
{
    ...
    {
        LogPrint("coinmixing", "Transaction 2: %s\n", txNew.ToString());

        // See if the transaction is valid
        if (!txNew.AcceptToMemoryPool(false, true, true)) {
            LogPrintf("CCoinMixingPool::Check() - CommitTransaction : Error: Transaction not valid\n");
            SetNull();
        ...
        LogPrintf("CCoinMixingPool::Check() -- IS MASTER -- TRANSMITTING COINMIXING\n");
        ...
        if (!obfuScationSigner.SetKey(strMasterNodePrivKey, strError, key2, pubkey2)) {
            LogPrintf("CCoinMixingPool::Check() - ERROR: Invalid Masternodeprivkey: '%s'\n", strError);
            return;
        }

        if (!obfuScationSigner.SignMessage(strMessage, strError, vchSig, key2)) {
            LogPrintf("CCoinMixingPool::Check() - Sign message failed\n");
            return;
        }

        if (!obfuScationSigner.VerifyMessage(pubkey2, vchSig, strMessage, strError)) {
            LogPrintf("CCoinMixingPool::Check() - Verify message failed\n");
            return;
        }

        if (!mapCoinMixingBroadcastTxes.count(txNew.GetHash())) {
            ...
            mapCoinMixingBroadcastTxes.insert(make_pair(txNew.GetHash(), dstx));
        }
        ...
        // Reset
        LogPrint("coinmixing", "CCoinMixingPool::Check() -- COMPLETED -- RESETTING\n");
        SetNull();
        RelayStatus(sessionID, GetState(), GetEntriesCount(), MASTERNODE_RESET);
    }
}
...
//
// Why bother? CoinMixing uses collateral to ensure abuse to the process is kept to a minimum.
// The submission and signing stages in CoinMixing are completely separate. In the cases where
...
//
void CCoinMixingPool::ChargeFees()
{
    ...
    if (state == POOL_STATUS_ACCEPTING_ENTRIES) {
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            if (!found) {
                LogPrintf("CCoinMixingPool::ChargeFees -- found uncooperative node (didn't send transaction). Found offence.\n");
                offences++;
            }
        }
    }
    ...
    if (state == POOL_STATUS_SIGNING) {
        ...
        BOOST_FOREACH (const CObfuScationEntry v, entries) {
            BOOST_FOREACH (const CTxDSIn s, v.sev) {
                if (!s.fHasSig) {
                    LogPrintf("CCoinMixingPool::ChargeFees -- found uncooperative node (didn't sign). Found offence\n");
                    offences++;
                }
            }
        }
    ...
    if (state == POOL_STATUS_ACCEPTING_ENTRIES) {
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            // This queue entry didn't send us the promised transaction
            if (!found && r > target) {
                LogPrintf("CCoinMixingPool::ChargeFees -- found uncooperative node (didn't send transaction). charging fees.\n");
                ...
                if (!wtxCollateral.AcceptToMemoryPool(true)) {
                    ...
                    LogPrintf("CCoinMixingPool::ChargeFees() : Error: Transaction not valid");
                }
        ...
        BOOST_FOREACH (const CObfuScationEntry v, entries) {
            BOOST_FOREACH (const CTxDSIn s, v.sev) {
                if (!s.fHasSig && r > target) {
                    LogPrintf("CCoinMixingPool::ChargeFees -- found uncooperative node (didn't sign). charging fees.\n");
...
//  - CoinMixing is completely free, to pay miners we randomly pay the collateral of users.
void CCoinMixingPool::ChargeRandomFees()
{
    if (fMasterNode) {
        int i = 0;
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                Being that CoinMixing has "no fees" we need to have some kind of cost associated
                with using it to stop abuse. Otherwise it could serve as an attack vector and
                allow endless transaction that would bloat bitcoinpay and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 BPAY per transaction on average.
            */
            if (r <= 10) {
                LogPrintf("CCoinMixingPool::ChargeRandomFees -- charging random fees. %u\n", i);
                ...
                if (!wtxCollateral.AcceptToMemoryPool(true)) {
                    // This must not fail. The transaction has already been signed and recorded.
                    LogPrintf("CCoinMixingPool::ChargeRandomFees() : Error: Transaction not valid");
                }
                wtxCollateral.RelayWalletTransaction();
            }
        }
...
//
// Check for various timeouts (queue objects, CoinMixing, etc)
//
void CCoinMixingPool::CheckTimeout()
{
    if (!fEnableCoinMixing && !fMasterNode) return;

    // catching hanging sessions
    if (!fMasterNode) {
        switch (state) {
        case POOL_STATUS_TRANSMISSION:
            LogPrint("coinmixing", "CCoinMixingPool::CheckTimeout() -- Session complete -- Running Check()\n");
            Check();
            break;
        case POOL_STATUS_ERROR:
            LogPrint("coinmixing", "CCoinMixingPool::CheckTimeout() -- Pool error -- Running Check()\n");
            Check();
            break;
        case POOL_STATUS_SUCCESS:
            LogPrint("coinmixing", "CCoinMixingPool::CheckTimeout() -- Pool success -- Running Check()\n");
            Check();
            break;
        }
    }

    // check CoinMixing queue objects for timeouts
    int c = 0;
    vector<CCoinMixingQueue>::iterator it = vecCoinMixingQueue.begin();
    while (it != vecCoinMixingQueue.end()) {
        if ((*it).IsExpired()) {
            LogPrint("coinmixing", "CCoinMixingPool::CheckTimeout() : Removing expired queue entry - %d\n", c);
            it = vecCoinMixingQueue.erase(it);
        } else
            ++it;
        c++;
    }
    ...
    if (state == POOL_STATUS_ACCEPTING_ENTRIES || state == POOL_STATUS_QUEUE) {
        ...
        while (it2 != entries.end()) {
            if ((*it2).IsExpired()) {
                LogPrint("coinmixing", "CCoinMixingPool::CheckTimeout() : Removing expired entry - %d\n", c);
        ...
        if (GetTimeMillis() - lastTimeChanged >= (COINMIXING_QUEUE_TIMEOUT * 1000) + addLagTime) {
            UnlockCoins();
            SetNull();
        }
    } else if (GetTimeMillis() - lastTimeChanged >= (COINMIXING_QUEUE_TIMEOUT * 1000) + addLagTime) {
        LogPrint("coinmixing", "CCoinMixingPool::CheckTimeout() -- Session timed out (%ds) -- resetting\n", COINMIXING_QUEUE_TIMEOUT);
        UnlockCoins();
        SetNull();
    ...
    if (state == POOL_STATUS_SIGNING && GetTimeMillis() - lastTimeChanged >= (COINMIXING_SIGNING_TIMEOUT * 1000) + addLagTime) {
        LogPrint("coinmixing", "CCoinMixingPool::CheckTimeout() -- Session timed out (%ds) -- restting\n", COINMIXING_SIGNING_TIMEOUT);
        ChargeFees();
        UnlockCoins();
        SetNull();
...
//
void CCoinMixingPool::CheckForCompleteQueue()
{
    if (!fEnableCoinMixing && !fMasterNode) return;
...
bool CCoinMixingPool::SignatureValid(const CScript& newSig, const CTxIn& newVin)
{
    CMutableTransaction txNew;
    txNew.vin.clear();
    txNew.vout.clear();
    ...
    if (found >= 0) { //might have to do this one input at a time?
        ...
        LogPrint("coinmixing", "CCoinMixingPool::SignatureValid() - Sign with sig %s\n", newSig.ToString().substr(0, 24));
        if (!VerifyScript(txNew.vin[n].scriptSig, sigPubKey, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_STRICTENC, MutableTransactionSignatureChecker(&txNew, n))) {
            LogPrint("coinmixing", "CCoinMixingPool::SignatureValid() - Signing - Error signing input %u\n", n);
            return false;
        }
    }

    LogPrint("coinmixing", "CCoinMixingPool::SignatureValid() - Signing - Successfully validated input\n");
    return true;
}

// check to make sure the collateral provided by the client is valid
bool CCoinMixingPool::IsCollateralValid(const CTransaction& txCollateral)
{
    ...
    BOOST_FOREACH (const CTxOut o, txCollateral.vout) {
        ...
        if (!o.scriptPubKey.IsNormalPaymentScript()) {
            LogPrintf("CCoinMixingPool::IsCollateralValid - Invalid Script %s\n", txCollateral.ToString());
            return false;
        }
    }
    ...
    if (missingTx) {
        LogPrint("coinmixing", "CCoinMixingPool::IsCollateralValid - Unknown inputs in collateral transaction - %s\n", txCollateral.ToString());
        return false;
    }

    //collateral transactions are required to pay out COINMIXING_COLLATERAL as a fee to the miners
    if (nValueIn - nValueOut < COINMIXING_COLLATERAL) {
        LogPrint("coinmixing", "CCoinMixingPool::IsCollateralValid - did not include enough fees in transaction %d\n%s\n", nValueOut - nValueIn, txCollateral.ToString());
        return false;
    }

    LogPrint("coinmixing", "CCoinMixingPool::IsCollateralValid %s\n", txCollateral.ToString());

    {
        LOCK(cs_main);
        CValidationState state;
        if (!AcceptableInputs(mempool, state, txCollateral, true, NULL)) {
            if (fDebug) LogPrintf("CCoinMixingPool::IsCollateralValid - didn't pass IsAcceptable\n");
            return false;
        }
    }
...
//
bool CCoinMixingPool::AddEntry(const std::vector<CTxIn>& newInput, const int64_t& nAmount, const CTransaction& txCollateral, const std::vector<CTxOut>& newOutput, int& errorID)
{
    if (!fMasterNode) return false;

    BOOST_FOREACH (CTxIn in, newInput) {
        if (in.prevout.IsNull() || nAmount < 0) {
            LogPrint("coinmixing", "CCoinMixingPool::AddEntry - input not valid!\n");
    ...
    if (!IsCollateralValid(txCollateral)) {
        LogPrint("coinmixing", "CCoinMixingPool::AddEntry - collateral not valid!\n");
    ...
    if ((int)entries.size() >= GetMaxPoolTransactions()) {
        LogPrint("coinmixing", "CCoinMixingPool::AddEntry - entries is full!\n");
    ...
    BOOST_FOREACH (CTxIn in, newInput) {
        LogPrint("coinmixing", "looking for vin -- %s\n", in.ToString());
        BOOST_FOREACH (const CObfuScationEntry& v, entries) {
            BOOST_FOREACH (const CTxDSIn& s, v.sev) {
                if ((CTxIn)s == in) {
                    LogPrint("coinmixing", "CCoinMixingPool::AddEntry - found in vin\n");
    ...
    LogPrint("coinmixing", "CCoinMixingPool::AddEntry -- adding %s\n", newInput[0].ToString());
    errorID = MSG_ENTRIES_ADDED;

    return true;
}

bool CCoinMixingPool::AddScriptSig(const CTxIn& newVin)
{
    LogPrint("coinmixing", "CCoinMixingPool::AddScriptSig -- new sig  %s\n", newVin.scriptSig.ToString().substr(0, 24));


    BOOST_FOREACH (const CObfuScationEntry& v, entries) {
        BOOST_FOREACH (const CTxDSIn& s, v.sev) {
            if (s.scriptSig == newVin.scriptSig) {
                LogPrint("coinmixing", "CCoinMixingPool::AddScriptSig - already exists\n");
                return false;
            }
        }
    }
    ...
    if (!SignatureValid(newVin.scriptSig, newVin)) {
        LogPrint("coinmixing", "CCoinMixingPool::AddScriptSig - Invalid Sig\n");
        return false;
    }

    LogPrint("coinmixing", "CCoinMixingPool::AddScriptSig -- sig %s\n", newVin.ToString());

    BOOST_FOREACH (CTxIn& vin, finalTransaction.vin) {
        if (newVin.prevout == vin.prevout && vin.nSequence == newVin.nSequence) {
            vin.scriptSig = newVin.scriptSig;
            vin.prevPubKey = newVin.prevPubKey;
            LogPrint("coinmixing", "CObfuScationPool::AddScriptSig -- adding to finalTransaction  %s\n", newVin.scriptSig.ToString().substr(0, 24));
        }
    }
    for (unsigned int i = 0; i < entries.size(); i++) {
        if (entries[i].AddSig(newVin)) {
            LogPrint("coinmixing", "CObfuScationPool::AddScriptSig -- adding  %s\n", newVin.scriptSig.ToString().substr(0, 24));
            return true;
        }
    }

    LogPrintf("CCoinMixingPool::AddScriptSig -- Couldn't set sig!\n");
    return false;
}

// Check to make sure everything is signed
bool CCoinMixingPool::SignaturesComplete()
{
    BOOST_FOREACH (const CObfuScationEntry& v, entries) {
        BOOST_FOREACH (const CTxDSIn& s, v.sev) {
...
//
// Execute a CoinMixing denomination via a Masternode.
// This is only ran from clients
//
void CCoinMixingPool::SendCoinMixingDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, int64_t amount)
{
    if (fMasterNode) {
        LogPrintf("CCoinMixingPool::SendCoinMixingDenominate() - CoinMixing from a Masternode is not supported currently.\n");
        return;
    }

    if (txCollateral == CMutableTransaction()) {
        LogPrintf("CCoinMixingPool:SendCoinMixingDenominate() - CoinMixing collateral not set");
        return;
    }
    ...
    if (!sessionFoundMasternode) {
        LogPrintf("CCoinMixingPool::SendCoinMixingDenominate() - No Masternode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return;
    }
    ...
    if (!CheckDiskSpace()) {
        UnlockCoins();
        SetNull();
        fEnableCoinMixing = false;
        LogPrintf("CCoinMixingPool::SendCoinMixingDenominate() - Not enough disk space, disabling CoinMixing.\n");
        return;
    }

    UpdateState(POOL_STATUS_ACCEPTING_ENTRIES);

    LogPrintf("CCoinMixingPool::SendCoinMixingDenominate() - Added transaction to pool.\n");
...
// Incoming message from Masternode updating the progress of CoinMixing
//    newAccepted:  -1 mean's it'n not a "transaction accepted/not accepted" message, just a standard update
//                  0 means transaction was not accepted
//                  1 means transaction was accepted

bool CCoinMixingPool::StatusUpdate(int newState, int newEntriesCount, int newAccepted, int& errorID, int newSessionID)
{
    if (fMasterNode) return false;
    ...
    if (newAccepted != -1) {
        ...
        if (newAccepted == 1 && newSessionID != 0) {
            ...
            LogPrintf("CCoinMixingPool::StatusUpdate - set sessionID to %d\n", sessionID);
    ...
    if (newState == POOL_STATUS_ACCEPTING_ENTRIES) {
        if (newAccepted == 1) {
            LogPrintf("CCoinMixingPool::StatusUpdate - entry accepted! \n");
            sessionFoundMasternode = true;
            //wait for other users. Masternode will report when ready
            UpdateState(POOL_STATUS_QUEUE);
        } else if (newAccepted == 0 && sessionID == 0 && !sessionFoundMasternode) {
            LogPrintf("CCoinMixingPool::StatusUpdate - entry not accepted by Masternode \n");
            UnlockCoins();
            UpdateState(POOL_STATUS_ACCEPTING_ENTRIES);
...
//
bool CCoinMixingPool::SignFinalTransaction(CTransaction& finalTransactionNew, CNode* node)
{
    if (fMasterNode) return false;

    finalTransaction = finalTransactionNew;
    LogPrintf("CCoinMixingPool::SignFinalTransaction %s", finalTransaction.ToString());
        ...
        BOOST_FOREACH (const CTxDSIn s, e.sev) {
            ...
            if (mine >= 0) { //might have to do this one input at a time?
                ...
                if (foundOutputs < targetOuputs || nValue1 != nValue2) {
                    ...
                    LogPrintf("CCoinMixingPool::Sign - My entries are not correct! Refusing to sign. %d entries %d target. \n", foundOutputs, targetOuputs);
                    UnlockCoins();
                    SetNull();
...
void CCoinMixingPool::NewBlock()
{
    LogPrint("coinmixing", "CCoinMixingPool::NewBlock \n");
...
// CoinMixing transaction was completed (failed or successful)
void CCoinMixingPool::CompletedTransaction(bool error, int errorID)
{
    if (fMasterNode) return;
...
void CCoinMixingPool::ClearLastMessage()
{
    lastMessage = "";
}

//
// Passively run CoinMixing in the background to anonymize funds based on the given configuration.
//
// This does NOT run by default for daemons, only for QT.
//
bool CCoinMixingPool::DoAutomaticDenominating(bool fDryRun)
{
    if (!fEnableCoinMixing) return false;
    if (fMasterNode) return false;
    if (state == POOL_STATUS_ERROR || state == POOL_STATUS_SUCCESS) return false;
    ...
    if (chainActive.Tip()->nHeight - cachedLastSuccess < minBlockSpacing) {
        LogPrintf("CCoinMixingPool::DoAutomaticDenominating - Last successful CoinMixing action was too recent\n");
        strAutoDenomResult = _("Last successful CoinMixing action was too recent.");
        return false;
    }

    if (mnodeman.size() == 0) {
        LogPrint("coinmixing", "CCoinMixingPool::DoAutomaticDenominating - No Masternodes detected\n");
    ...
    // should not be less than fees in COINMIXING_COLLATERAL + few (lets say 5) smallest denoms
    CAmount nLowestDenom = COINMIXING_COLLATERAL + obfuScationDenominations[obfuScationDenominations.size() - 1] * 5;

    // if there are no OBF collateral inputs yet
    if (!pwalletMain->HasCollateralInputs())
        // should have some additional amount for them
        nLowestDenom += COINMIXING_COLLATERAL * 4;

    CAmount nBalanceNeedsAnonymized = nAnonymizeBitcoinPayAmount * COIN - pwalletMain->GetAnonymizedBalance();

    // if balanceNeedsAnonymized is more than pool max, take the pool max
    if (nBalanceNeedsAnonymized > COINMIXING_POOL_MAX) nBalanceNeedsAnonymized = COINMIXING_POOL_MAX;
    ...
    LogPrint("coinmixing", "DoAutomaticDenominating : nLowestDenom=%d, nBalanceNeedsAnonymized=%d\n", nLowestDenom, nBalanceNeedsAnonymized);

    // select coins that should be given to the pool
    if (!pwalletMain->SelectCoinsDark(nValueMin, nBalanceNeedsAnonymized, vCoins, nValueIn, 0, nCoinMixingRounds)) {
        nValueIn = 0;
        vCoins.clear();
    ...
    if (!sessionFoundMasternode) {
        ...
        if (nUseQueue > 33) {
            ...
            BOOST_FOREACH (CCoinMixingQueue& dsq, vecCoinMixingQueue) {
                ...
                if (!pwalletMain->SelectCoinsByDenominations(dsq.nDenom, nValueMin, nBalanceNeedsAnonymized, vTempCoins, vTempCoins2, nValueIn, 0, nCoinMixingRounds)) {
...
bool CCoinMixingPool::PrepareCoinMixingDenominate()
{
    std::string strError = "";
    // Submit transaction to the pool if we get here
    // Try to use only inputs with the same number of rounds starting from lowest number of rounds possible
    for (int i = 0; i < nCoinMixingRounds; i++) {
        strError = pwalletMain->PrepareCoinMixingDenominate(i, i + 1);
        LogPrintf("DoAutomaticDenominating : Running CoinMixing denominate for %d rounds. Return '%s'\n", i, strError);
        if (strError == "") return true;
    }

    // We failed? That's strange but let's just make final attempt and try to mix everything
    strError = pwalletMain->PrepareCoinMixingDenominate(0, nCoinMixingRounds);
    LogPrintf("DoAutomaticDenominating : Running CoinMixing denominate for all rounds. Return '%s'\n", strError);
    if (strError == "") return true;
...
bool CCoinMixingPool::SendRandomPaymentToSelf()
{
...
bool CCoinMixingPool::MakeCollateralAmounts()
{
    CWalletTx wtx;
    int64_t nFeeRet = 0;
    std::string strFail = "";
    ...
    vecSend.push_back(make_pair(scriptCollateral, COINMIXING_COLLATERAL * 4));
...
bool CCoinMixingPool::CreateDenominated(int64_t nTotalValue)
{
    CWalletTx wtx;
    int64_t nFeeRet = 0;
    std::string strFail = "";
    ...
    if (!pwalletMain->HasCollateralInputs()) {
        vecSend.push_back(make_pair(scriptCollateral, COINMIXING_COLLATERAL * 4));
        nValueLeft -= COINMIXING_COLLATERAL * 4;
    }
    ...
    BOOST_REVERSE_FOREACH (int64_t v, obfuScationDenominations) {
        ...
        while (nValueLeft - v >= COINMIXING_COLLATERAL && nOutputs <= 10) {
            CScript scriptDenom;
            CPubKey vchPubKey;
...
bool CCoinMixingPool::IsCompatibleWithEntries(std::vector<CTxOut>& vout)
{
...
bool CCoinMixingPool::IsCompatibleWithSession(int64_t nDenom, CTransaction txCollateral, int& errorID)
{
    if (nDenom == 0) return false;

    LogPrintf("CCoinMixingPool::IsCompatibleWithSession - sessionDenom %d sessionUsers %d\n", sessionDenom, sessionUsers);

    if (!unitTest && !IsCollateralValid(txCollateral)) {
        LogPrint("coinmixing", "CCoinMixingPool::IsCompatibleWithSession - collateral not valid!\n");
        errorID = ERR_INVALID_COLLATERAL;
        return false;
    }
    ...
    if ((state != POOL_STATUS_ACCEPTING_ENTRIES && state != POOL_STATUS_QUEUE) || sessionUsers >= GetMaxPoolTransactions()) {
        ...
        LogPrintf("CCoinMixingPool::IsCompatibleWithSession - incompatible mode, return false %d %d\n", state != POOL_STATUS_ACCEPTING_ENTRIES, sessionUsers >= GetMaxPoolTransactions());
        return false;
    }
...
void CCoinMixingPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    // Function returns as follows:
    //
    // bit 0 - 100BPAY+1 ( bit on if present )
    // bit 1 - 10BPAY+1
    // bit 2 - 1BPAY+1
    // bit 3 - .1BPAY+1
    // bit 3 - non-denom
...
int CCoinMixingPool::GetDenominations(const std::vector<CTxDSOut>& vout)
{
...
int CCoinMixingPool::GetDenominations(const std::vector<CTxOut>& vout, bool fSingleRandomDenom)
{
...
int CCoinMixingPool::GetDenominationsByAmounts(std::vector<int64_t>& vecAmount)
{
    CScript e = CScript();
...
int CCoinMixingPool::GetDenominationsByAmount(int64_t nAmount, int nDenomTarget)
{
    CScript e = CScript();
...
std::string CCoinMixingPool::GetMessageByID(int messageID)
{
    switch (messageID) {
    ...
    case ERR_MAXIMUM:
        return _("Value more than CoinMixing pool maximum allows.");
    case ERR_MN_LIST:
    ...
    case ERR_RECENT:
        return _("Last CoinMixing was too recent.");
    case ERR_SESSION:
...
bool CCoinMixingQueue::Sign()
{
    ...
    if (!obfuScationSigner.SetKey(strMasterNodePrivKey, errorMessage, key2, pubkey2)) {
        LogPrintf("CCoinMixingQueue():Relay - ERROR: Invalid Masternodeprivkey: '%s'\n", errorMessage);
        return false;
    }

    if (!obfuScationSigner.SignMessage(strMessage, errorMessage, vchSig, key2)) {
        LogPrintf("CCoinMixingQueue():Relay - Sign message failed");
        return false;
    }

    if (!obfuScationSigner.VerifyMessage(pubkey2, vchSig, strMessage, errorMessage)) {
        LogPrintf("CCoinMixingQueue():Relay - Verify message failed");
        return false;
    }
...
bool CCoinMixingQueue::CheckSignature()
{
    ...
    if (pmn != NULL) {
        ...
        if (!obfuScationSigner.VerifyMessage(pmn->pubKeyMasternode, vchSig, strMessage, errorMessage)) {
            return error("CCoinMixingQueue::CheckSignature() - Got bad Masternode address signature %s \n", vin.ToString().c_str());
        }
...
void CCoinMixingPool::RelayFinalTransaction(const int sessionID, const CTransaction& txNew)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH (CNode* pnode, vNodes) {
...
void CCoinMixingPool::RelayIn(const std::vector<CTxDSIn>& vin, const int64_t& nAmount, const CTransaction& txCollateral, const std::vector<CTxDSOut>& vout)
{
    if (!pSubmittedToMasternode) return;
...
void CCoinMixingPool::RelayStatus(const int sessionID, const int newState, const int newEntriesCount, const int newAccepted, const int errorID)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH (CNode* pnode, vNodes)
...
void CCoinMixingPool::RelayCompletedTransaction(const int sessionID, const bool error, const int errorID)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH (CNode* pnode, vNodes)
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("bitcoinpay-coinmixing");
