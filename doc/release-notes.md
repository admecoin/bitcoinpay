BitcoinPay version 2.3.0 is now available from:
...
BitcoinPay is extensively tested on multiple operating systems using
...
BitcoinPay should also work on most other Unix-like systems but is not
...
BitcoinPay now enables bloom filters by default to support SPV clients like mobile wallets. This feature can be disabled by using the `-peerbloomfilters` option on startup.
